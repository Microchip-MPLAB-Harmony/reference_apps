/******************************************************************************
  USB OHCI Driver Interface Header File.
...
/*DOM-IGNORE-BEGIN*/extern DRV_USB_HOST_INTERFACE gDrvUSBOHCIInterface;/*DOM-IGNORE-END */
#define DRV_USB_OHCI_INTERFACE /*DOM-IGNORE-BEGIN*/&gDrvUSBOHCIInterface/*DOM-IGNORE-END */
...
/* USB Driver Module Index Definition.
  ...
  Description:
    ...
    SYS_MODULE_INDEX parameter of the DRV_USB_OHCI_Initialize and
    DRV_USB_OHCI_Open functions should be set to these values to identify an
    instance of the driver. 

  Remarks:
    These constants should be used in place of hard-coded numeric literals and
    should be passed into the DRV_USB_OHCI_Initialize and DRV_USB_OHCI_Open
...
#define DRV_USB_OHCI_INDEX_0         0
#define DRV_USB_OHCI_INDEX_1         1
#define DRV_USB_OHCI_INDEX_2         2
...
typedef uintptr_t DRV_USB_OHCI_PIPE_HANDLE;
...
#define DRV_USB_OHCI_PIPE_HANDLE_INVALID ((DRV_USB_OHCI_PIPE_HANDLE)(-1))
...
typedef bool (* DRV_USB_OHCI_PORT_OVER_CURRENT_DETECT)(uint8_t port);
...
typedef void (* DRV_USB_OHCI_PORT_POWER_ENABLE)
(
    uint8_t port, 
    bool control
);
...
typedef void(* DRV_USB_OHCI_PORT_INDICATION)
(
    uint8_t port, 
...
/* USB Device Driver Initialization Data.
  ...
  Description:
    ...
    data, must be passed into the DRV_USB_OHCI_Initialize function.
...
typedef struct
{
    ...
    /* The OHCI controller may support multiple ports. This bit map specifies
...
/* Function:
  ...
  Remarks:
    ...
    DRV_USB_OHCI_Deinitialize is called to deinitialize the driver instance. 
*/
...
/* Function:
  ...
  Precondition:
    The DRV_USB_OHCI_Initialize function must have been called before calling this
    function.

  Parameters:
    object - Driver object handle, returned from the DRV_USB_OHCI_Initialize function.
  ...
  Example:
    ...
    status = DRV_USB_OHCI_Status(object);
...
/* Function:
  ...
  Precondition:
    The DRV_USB_OHCI_Initialize function must have been called for the specified
    USB Driver instance.

  Parameters:
    object - Object handle for the specified driver instance (returned from
    DRV_USB_OHCI_Initialize function).
  ...
  Example:
    <code>
    SYS_MODULE_OBJ      object;     // Returned from DRV_USB_OHCI_Initialize

    while (true)
    {
        DRV_USB_OHCI_Tasks(object);
...
/* Function:
  ...
  Example:
    ...
    handle = DRV_USB_OHCI_Open(DRV_USB_OHCI_INDEX_0, DRV_IO_INTENT_EXCLUSIVE|
...
// *****************************************************************************
/* Function:
    void DRV_USB_OHCI_Close( DRV_HANDLE handle )
  ...
  Precondition:
    The DRV_USB_OHCI_Initialize function must have been called for the specified
    USB Driver instance. DRV_USB_OHCI_Open function must have been called to obtain
  ...
  Example:
    <code>
    DRV_HANDLE handle;  // Returned from DRV_USB_OHCI_Open

    DRV_USB_OHCI_Close(handle);
    </code>
  ...
  Remarks:
    ...
    obtained by calling DRV_USB_OHCI_Open function before the caller may use the
    driver again.
*/
...
/* Function:
  ...
  Example:
    ...
    // DRV_USB_OHCI_EventsEnable function can be called to disable and enable
    // events.
    ...
    eventsWereEnabled = DRV_USB_OHCI_EventsDisable(driverHandle);
...
/* Function:
  ...
  Returns:
    * DRV_USB_OHCI_PIPE_HANDLE_INVALID - The pipe could not be created.
  ...
  Example:
    ...
    pipeHandle = DRV_USB_OHCI_PipeSetup(driverHandle, 0x02, 0x14, 0x03, 0x01, USB_TRANSFER_TYPE_BULK, 0, 64, USB_SPEED_FULL); 
...
/* Function:
  ...
  Example:
    ...
    // DRV_USB_OHCI_PortResetIsComplete functions are called to complete a
    // port reset sequence.

    DRV_HANDLE driverHandle;

    // Reset Port 0.
    DRV_USB_OHCI_PortReset(driverHandle, 0);

    // Check if the Reset operation has completed.
    if(DRV_USB_OHCI_PortResetIsComplete(driverHandle, 0) == false)
    {
        // This means that the Port Reset operation has not completed yet. The
        // DRV_USB_OHCI_PortResetIsComplete function should be called
...
bool DRV_USB_OHCI_PortResetIsComplete(DRV_HANDLE handle, uint8_t port );

// ****************************************************************************
/* Function:
    USB_ERROR DRV_USB_OHCI_PortResume
    (
        DRV_HANDLE handle, 
        uint8_t port
    );
...
/* Function:
  ...
  Example:
    <code>
    // This code shows how DRV_USB_OHCI_PortNumbersGet function can
    // be called to obtain the number of Root hub ports.

    DRV_HANDLE driverHandle;
    uint8_t nPorts;

    nPorts = DRV_USB_OHCI_PortNumbersGet(driverHandle);
...
// ****************************************************************************
/* Function:
    void DRV_USB_OHCI_OperationEnable
    (
        DRV_HANDLE handle, 
        bool enable
    );
  ...
  Example:
    <code>
    // This code shows how the DRV_USB_OHCI_OperationEnable and the
    // DRV_USB_OHCI_OperationIsEnabled functions are called to enable
    // the Root hub operation.

    DRV_HANDLE driverHandle;

    // Enable Root hub operation.
    DRV_USB_OHCI_OperationEnable(driverHandle);

    // Wait till the Root hub operation is enabled.  
    if(DRV_USB_OHCI_OperationIsEnabled(driverHandle) == false)
    {
        // The operation has not completed. Call the
        // DRV_USB_OHCI_OperationIsEnabled function again to check if
        // the operation has completed. Note that the DRV_USB_OHCI_Tasks function
...
/* Function:
  ...
  Description:
    This function returns true if the DRV_USB_OHCI_OperationEnable
...
/* Function:
  ...
  Example:
    ...
    // DRV_USB_OHCI_Initialize function. The usbHostDeviceInfo
    ...
    DRV_USB_OHCI_Initialize(drvHandle, usbHostDeviceInfo);
...
/* Function:
  ...
  Example:
    ...
    // This code shows how the USB Host Layer calls the
    // DRV_USB_OHCI_EndpointToggleClear function. 

    DRV_USB_OHCI_PIPE_HANDLE pipeHandle ;

    DRV_USB_OHCI_EndpointToggleClear(pipeHandle);
...
/* Function:
  ...
  Parameters:
    drvIndex - OHCI driver instance index.

  Example:
    <code>

    if(DRV_USB_OHCI_IsCompanionControllerReady(DRV_USB_OHCI_INDEX_0))
    {
        // Ownership can be assigned to the driver 
    }
