...
{
     if( SYS_FS_FileStat((char *) fname, &(AppDataAudioPlayerPtr->fileStatus)) == SYS_FS_RES_FAILURE )
     {
             return false;
     }
     else
...
SYS_FS_HANDLE APP_SDCARD_AUDIO_Card_OpenFile ( const int8_t *fname )
{
    ...
    fileHandle = SYS_FS_FileOpen((char *) fname, (SYS_FS_FILE_OPEN_READ_PLUS));
        ...
        bool isRoot)
{
     ...
     ret = APP_SDCARD_AUDIO_Card_FS_ReadDirFlat((int8_t *) node.path, &totalDir, dirTable, isRoot);
...
{
    SYS_FS_RESULT ret;
    AppDataAudioCardData.dirHandle = SYS_FS_DirOpen((char *) fname);
                 ...
                 if(AppDataAudioCardData.dirstat.fattrib != SYS_FS_ATTR_DIR)
                 {
                    if(APP_SDCARD_AUDIO_Card_isSupportedAudioFile((int8_t *) (AppDataAudioCardData.dirstat.fname)))
                    {
                        strcpy((char *) (AppSdCardAudioCardFilesTable
                                [AppDataAudioPlayerPtr->totalAudioFiles].path), (char *) fname);
                        if(!isRoot)
                        {
...
void APP_SDCARD_AUDIO_RandomTrackSet(uint32_t trackId)
{
    if(   AppSdCardAudioData.player.currentSongIdx != trackId
       && AppSdCardAudioData.player.totalAudioFiles > trackId
    )
    {
